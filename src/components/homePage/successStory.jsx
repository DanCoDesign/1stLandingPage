...
function StoryItem({ title, text, img }) {
    return (
        <>
            <div className="flex justify-between lg:min-w-[380px] lg:min-h-[420px]">
                <div className="flex flex-col text-left w-auto max-w-[60%] lg:mr-10">
                    <h3 className="my-8 text-2xl font-bold leading-snug tracking-tight text-gray-800 lg:leading-tight lg:text-4xl dark:text-white">{title}</h3>
                    <p className="mb-16 text-xl lg:leading-[3rem] text-gray-700 lg:text-2xl xl:text-3xl dark:text-gray-300">{text}</p>
                    <AmazonLogo />
                    <Link
                        href="#"
                        target="_blank"
                        rel="noopener"
                        className="py-4 text-regular font-normal text-[#2E4FD7] dark:text-[#2E8FD7] flex items-center">
                        Read Complete Case Study

                    </Link>
                </div>
                <div className="hidden lg:flex relative w-[380px] h-[420px]">
                    <Image
                        src={img}
                        width={380}
                        height={420}
                        alt="Success Illustration"
                        className="absolute bg-amber-500 m-auto left-0 right-0 top-0 bottom-0"
                    />
                </div>
...
export default function SuccessStory() {
    ...
    {
        "title": "Best in the business. Period.",
                ...
                <Carousel autoPlay
                    ...
                    {items.map((item) => (
                        <div key={item} className="">
                            <StoryItem title={item.title} text={item.text} img={item.img} />
...
function AmazonLogo() {
    return (
        <svg
            ...
            fill="none"
            viewBox="0 0 110 33">
            <g fill="#B4B4B4" clipPath="url(#clip0)">
                <path
                    fillRule="evenodd"
